pub fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    // Final padding uses at most two fixed stack blocks; hashing no longer allocates.
    let rem = chunks.remainder();
    let mut tail = [0u8; 128];
    tail[..rem.len()].copy_from_slice(rem);
    tail[rem.len()] = 0x80;
    let bit_len = (input.len() as u64).wrapping_mul(8);
    let used = if rem.len() <= 55 {
    ...
    for block in tail[..used].chunks_exact(64) {
        compress(&mut h, block.try_into().expect("64-byte padding block"));
    }
...
mod tests {
    ...
    fn standard_sha256_vectors() {
        ...
        assert_eq!(
            hex(&sha256(b"zero-os-userdb-v2\0root\0zero")),
            "517b0d2d99b8ecf6f12d64e23f0cf3679bab9440e24f48fd1ff6e44249a129e1"
        );
    }
}
